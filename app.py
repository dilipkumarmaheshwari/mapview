...
st.subheader("📏 Distance Summary")
for name, coord, geo_m, road_km in distances:
    st.markdown(f"**{name}** — Road: `{road_km:.2f} km`, Geo: `{geo_m:.0f} m`")
...
for radius, color in [(1000, "blue"), (3000, "purple"), (5000, "orange")]:
    folium.Circle(
        location=movable_point,
        radius=radius,
        color=color,
...
for name, coord, geo_m, road_km in distances:
    folium.Marker(
        location=coord,
        popup=f"{name}<br>Road: {road_km:.2f} km<br>Geo: {geo_m:.0f} m",
        icon=folium.Icon(color='green')
    ).add_to(m)

# Line to nearest place
folium.PolyLine([movable_point, nearest[1]], color='red', weight=4).add_to(m)
